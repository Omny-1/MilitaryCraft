...
/** Routes explosions into airship HP and shields raw display entities. */
public final class DamageListener implements Listener {

    private final AirshipRuntime plugin;

    public DamageListener(AirshipRuntime plugin) {
        this.plugin = plugin;
    }

    @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
    public void onEntityExplode(EntityExplodeEvent event) {
        if (plugin.airships().isInternalExplosion()
        ...
        if (src != null && src.getScoreboardTags().contains("antiaircraft_entity")) {
            plugin.airships().damageShipsFromAntiAir(event.getLocation());
            return;
        }
        plugin.airships().damageShipsFromExplosion(event.getLocation(), powerFor(src));
    }
    ...
    public void onBlockExplode(BlockExplodeEvent event) {
        ...
        plugin.airships().damageShipsFromExplosion(event.getBlock().getLocation().add(0.5, 0.5, 0.5), 5.0);
    }
    ...
    public void onDamage(EntityDamageEvent event) {
        ...
        if (ent instanceof Player player) {
            ...
            Airship seated = plugin.airships().byRider(player.getUniqueId());
            if (seated == null) {
                return;
            }
            ...
            if (event instanceof EntityDamageByEntityEvent ebe) {
                ...
                if (damager instanceof Player atk && plugin.airships().byRider(atk.getUniqueId()) != null) {
