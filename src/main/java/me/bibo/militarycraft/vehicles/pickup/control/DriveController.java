...
public final class DriveController {
    ...
    private static final double BLOCK_CHECK_HALF_LENGTH = (double)2.95f;
    ...
    public static void drive(Pickup pickup, Player driver, PickupConfig cfg) {
        ...
        if (Math.abs(speed) < 1.0E-6 && pickup.verticalVelocity() == 0.0 && !in.isForward() && !in.isBackward() && (g = DriveController.groundTopAt(world, oldX, oldZ, oldY, cfg)) != null && oldY - g <= cfg.maxStepUp + 0.001) {
            if (g < oldY - 1.0E-4) {
                anchor.setY(g.doubleValue());
            }
        ...
        if (speed != 0.0 && DriveController.leadingEdgeBlocked(world, newX, newZ, oldY, fx, fz, Math.signum(speed), cfg)) {
            newX = oldX;
            newZ = oldZ;
            pickup.setSpeed(0.0);
        }
        if ((support = DriveController.footprintGround(world, newX, newZ, fx, fz, oldY, cfg)) == null) {
        ...
        if (cfg.rammingEnabled && world.getFullTime() % 3L == 0L && Math.abs(pickup.speed()) >= cfg.rammingMinSpeed) {
            ...
            DriveController.ram(pickup, driver, fx * sign, fz * sign, cfg);
        }
        DriveController.movementEffects(pickup, cfg, drowned);
        if (world.getFullTime() % (long)cfg.hudInterval == 0L) {
            if (drowned) {
                Hud.sendSubmergedWarning(driver);
            } else {
    ...
    private static void ram(Pickup pickup, Player driver, double dirX, double dirZ, PickupConfig cfg) {
        ...
        Location centre = anchor.clone().add(0.0, (double)1.65f, 0.0);
        ...
        for (Entity e : pickup.world().getNearbyEntities(centre, radius, 2.449999976158142, radius)) {
            ...
            if (!(e instanceof Mob) && !(e instanceof Player) || e.equals((Object)driver) || e.getScoreboardTags().contains("pickupcraft_entity") || e.getVehicle() != null || living instanceof Player && ((p = (Player)living).getGameMode() == GameMode.CREATIVE || p.getGameMode() == GameMode.SPECTATOR)) continue;
            ...
            living.damage(cfg.rammingDamage, (Entity)driver);
    ...
    private static void movementEffects(Pickup pickup, PickupConfig cfg, boolean drowned) {
        ...
        if (cfg.dustTrail && time % 4L == 0L) {
            ...
            double side = 1.05;
            DriveController.spawnDust(world, anchor, rx * side, rz * side);
            DriveController.spawnDust(world, anchor, -rx * side, -rz * side);
        }
    ...
    private static void spawnDust(World world, Location anchor, double dx, double dz) {
        ...
        world.spawnParticle(Particle.BLOCK, at, 3, 0.22, 0.04, 0.22, 0.015, (Object)ground.getBlockData());
    }
    ...
    private static Double footprintGround(World world, double cx, double cz, double fx, double fz, double curY, PickupConfig cfg) {
        ...
        for (double[] o : FOOTPRINT_OFFSETS) {
            ...
            Double g = DriveController.groundTopAt(world, sx, sz, curY, cfg);
            if (g == null) continue;
            found = true;
    ...
    private static boolean leadingEdgeBlocked(World world, double cx, double cz, double curY, double fx, double fz, double moveSign, PickupConfig cfg) {
        ...
        double rx = fz;
        double rz = -fx;
        double leadX = cx + fx * (double)2.95f * moveSign;
        double leadZ = cz + fz * (double)2.95f * moveSign;
        for (double s : new double[]{-1.200000023841858, 0.0, 1.200000023841858}) {
            double sx = leadX + rx * s;
            double sz = leadZ + rz * s;
            Double g = DriveController.groundTopAt(world, sx, sz, curY, cfg);
            if (g == null || !(g - curY > cfg.maxStepUp) && !DriveController.blockedAhead(world, sx, g, sz)) continue;
            return true;
        }
        return false;
    }
